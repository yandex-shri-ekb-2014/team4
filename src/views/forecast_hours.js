...
var ForecastHoursView = TabPaneView.extend({
    ...
    render: function() {
        ...
        var makeSvg = function(params) {
            var margin = {
                    top: params.top || 20,
                    right: params.right || 30,
                    bottom: params.bottom || 30,
                    left: params.left || 20
                },

                width = params.width || 1400 - margin.left - margin.right,
                height = params.height || 250 - margin.top - margin.bottom;

            var tempPxArr = getPxArrOfTempArr(tempArr, height, 2);

            that.$el.html(forecastHoursTemplate());
            ...
            var svg = d3.select(params.element || '.forecast_hours__svg')
